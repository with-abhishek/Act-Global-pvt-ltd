...
function AddStudent() {
  ...
  const handleSubmit = (e) => {
    ...
    const invalidCourse = student.courses.some(
      (course) => !course.course_name || isNaN(parseFloat(course.course_fees))
    );
    if (invalidCourse) {
    ...
    StudentService.addStudent(payload)
      ...
      .catch((err) => {
        console.error('❌ Failed to submit student:', err.response?.data || err.message);
        alert('❌ Failed to submit student. Check console for details.');
      });
  };
